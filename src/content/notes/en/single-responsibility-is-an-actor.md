...
class RefundService {
  async submit(req: RefundRequest): Promise<RefundResult> {
    ...
    return {
      amount,
      status: 'done',
      display: `Refunded ¥${(amount / 100).toFixed(2)}`,
    }
  }
}
```
...
The cut fans out from one method to four files, one per group:

```mermaid
flowchart TB
    A["submit()<br/>one method, four groups"] --> B["Finance<br/>refund-amount"]
    A --> C["Compliance<br/>refund-audit"]
    A --> D["Operations<br/>refund-notifier"]
    A --> E["Support<br/>refund-view"]
```
...
```mermaid
flowchart TB
    R["Finance: change the rounding rule once"] --> F["RefundAmountFetcher"]
    F --> O["RefundAmountRounder"]
    O --> V["RefundAmountValidator"]
```
...
Those three classes are what the slogan produces when it is applied by its letter, and they carry both costs at once. The principle existed to protect one reason to change, and one reason now opens three files. The result is heavier than what you started with, because two indirect calls were added on the way. **Read as an acceptance criterion, the slogan produces code that misses the point of the principle and weighs more than code that ignored it.**
...
- **A responsibility is a person who will come asking, not a task.** Martin's own words in 2014: **this principle is about people**.
- **The test counts people, not methods.** Four hundred lines might serve one group. Twenty lines might have three groups watching them.
- **Cut along people and cohesion and coupling come with it.** Gather together the things that change for the same reasons, and separate those that change for different reasons.
- **"Does one thing" cannot be used to judge code.** Follow it to its limit and one reason to change turns into one reason to open three files.
- **Applying that slogan by the letter costs you twice.** It misses what the principle was protecting, and it adds two layers of indirection that were not there before.
- **Whether two groups are one or two comes down to one question: if one side adjusts, does the other follow on the same day?** The same person in two roles counts as two groups.
- **This is a judgement scale, not a measuring instrument.** The call can be wrong, and judgement is all there is.
