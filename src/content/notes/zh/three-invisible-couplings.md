...
```mermaid
flowchart LR
   A["直接调用<br/>只传 refundId<br/>第 6 档 · 数据耦合"] -->|"换成事件总线"| B["事件总线<br/>共享事件名 · payload · 顺序<br/>第 3 档 · 外部耦合"]
```

这不是说事件总线不能用。它说明**import 数量衡量的东西和等级表衡量的东西不是一回事**，删掉两行 import 的同时，可能把一个编译期可查的连接换成了运行期约定。
...
星形那张的线确实少了。代价是 Controller 成了唯一的分叉点：任何一个窗体的新情况、任何一个数据层方法的改动，都往同一个文件里落。`Form1` 和 `Form3` 从前互不相识，现在要排队改同一个文件。
...
这两次误判让我回头去查那张表的来历。它 1974 年要回答的是「一个模块改动时会不会牵动另一个」，档位描述的了解量，和 import 有几行本来就是两回事。传抄到「耦合越低越好」这句口号时，前半句丢了，只剩后半句的一个替代指标。**回溯原文的价值就在这一步：它能把一句口号换回当初那个具体麻烦，而麻烦是可以当场验的。**

现在再看一条依赖，我会先假设它的约定发生变化：编译器能不能列出所有受影响的位置？剩下那些必须靠搜索、文档和记忆才能补齐的部分，就是这条连接没有写在 import 里的成本。

「耦合越低越好」这句话单独放着没问题，一旦拿它当验收标准，它会系统性地奖励把依赖挪到看不见的地方的改动——事件总线、全局缓存、中间层，都能让指标变好看。**让指标好看和让系统变好是两件事，而且经常反向。**真正要判断的仍然是业务上这条连接该不该存在：暂时拿掉订阅方，主业务还成不成立。这个答案在业务里，等级表答不了。

## 这篇的结论

- **耦合量的是两个模块之间互相了解多少，不是 import 有几行。**import 数的是源文件里的符号，等级表问的是理解和修改一条连接要知道多少东西，两者会给出不同答案。
- **`dryRun` 这类参数落进控制耦合**，因为它额外暴露了被调方的控制流；`formatAmount(m, { showCurrency: true })` 描述输出长什么样，属于正常的数据耦合。
- **换成事件总线之后，档位往紧的方向走。**从只传 `refundId` 的数据耦合，变成共享事件名、payload 形状和执行顺序的外部耦合。这个改动在跨进程、跨团队场景下仍然值得做，只是它不叫解耦。
- **共享可变状态是第二档的公共耦合，比直接读写对方内部只松一点。**全文搜索能列出使用者，列不出读写所有权、先后顺序和原子性要求。
- **中间层不必然解耦。**网状变星形只改变了依赖图的形状，原来散在各处的了解量会集中到 Controller，需要同时打开的文件一个也没少。
- **依赖要指向更稳定的一侧，且不能成环。**成环的三个模块在变更影响上接近一个模块，这是少数能由工具直接查出的结构问题。
- **「耦合越低越好」当验收标准用会起反作用。**它奖励的是把依赖挪到工具看不见的地方，而不是把系统变好。
