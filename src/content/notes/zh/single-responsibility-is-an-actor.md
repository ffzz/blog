...
切法是从一个方法扇出到四个按人划分的文件：

```mermaid
flowchart TB
    A["submit()<br/>一个方法，四拨人"] --> B["财务<br/>refund-amount"]
    A --> C["合规<br/>refund-audit"]
    A --> D["运营<br/>refund-notifier"]
    A --> E["客服<br/>refund-view"]
```
...
```mermaid
flowchart TB
    R["财务：改一次舍入规则"] --> F["RefundAmountFetcher"]
...
照「只做一件事」拆出来的这三个类，恰好演示了硬套原则的两个代价：它既没有实现原则想保护的东西（一个理由修改，改成了一个理由打开三个文件），又比拆之前多出两处间接调用。**这句口号被当验收标准用的时候，产出的代码既不合原则本意，也比不合原则时更重。**
...
- **「职责」是一个会来提要求的人，不是一件事。**Martin 2014 年的原话是**这条原则是关于人的**。
- **判据是数人，不是数方法。****一个四百行的类可能只服务于一拨人，一个二十行的类可能同时被三拨人惦记。**
- **按人切开，顺带就得到了内聚和耦合。**把因为同一个理由而变的东西放在一起，把因为不同理由而变的东西分开。
- **「只做一件事」没法用来判断代码。**照它拆到极致，会得到一个理由要打开三个文件的结构。
- **硬套这句口号的代价是双倍的**：既没实现原则想保护的东西，又比不套时多出两层间接调用。
- **两拨人算一拨还是两拨，问一句就知道：一边调整，另一边会不会在同一天跟着改。****同一个人在不同角色下也算两拨人。**
- **这是判断量表，不是测量仪器。**这个判断会错，也只能靠判断。
