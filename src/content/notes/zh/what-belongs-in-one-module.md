...
```mermaid
flowchart BT
    C1["1 · 偶然<br/>没有理由"] --> C2["2 · 逻辑<br/>同类操作，靠参数选分支"]
    C2 --> C3["3 · 时间<br/>同一时刻执行"]
    C3 --> C4["4 · 通信<br/>操作同一份数据"]
    C4 --> C5["5 · 顺序<br/>前一步输出是后一步输入"]
    C5 --> C6["6 · 功能<br/>共同完成一件事"]
```
...
```mermaid
flowchart TB
    A["风控要求<br/>线下转账审批前加额度校验"] --> B["沿渠道切<br/>动 1 个文件"]
    A --> C["沿数据流切<br/>校验 · 扣款 · 记账 · 通知<br/>各动一处"]
```

图里没画出来的是：沿数据流切的时候，那四处改动散在四个模块里，谁也不挨着谁，改完还得凑在一起联调。
...
等级表要是当验收标准用，会引出一个反直觉的后果：为了让档位好看，人会倾向于把模块拆得更碎。可上面 `add` 和 `assertSameCurrency` 的例子说明，把共同完成一件事的两个函数拆开，档位是往下走的。**盯着档位调代码，可能越调越低。**判据不在表上，在这段代码所在的系统里——什么东西会一起变，只有那个业务知道，表上答不了。

这也是为什么值得回到 1974 年那篇原文。等级表是 Stevens、Myers、Constantine 为「怎么把一坨 Fortran 切成模块」做的打分表，每一档背后都是一次具体的切分决定。它被摘成「内聚越高越好」这句口号之后，当初那个具体麻烦就不在场了，拆得更碎于是一直看起来是对的，直到把 `add` 和币种校验拆开，才发现档位在往下走。**原则的灵魂是当初那个具体麻烦，麻烦得从业务里认，不能从表上抄。**

## 这篇的结论

- **内聚量的是「放在一起的理由」有多强，不是元素长得像不像。**`RefundHandler` 四个方法名字相似度拉满，按 1974 年那份等级表判，落在第二档。
- **判据是「这些东西是不是总是一起改」。**改一处会不会牵动其他几处，这是可以当场验的；相似度不是。
- **六档是有序的台阶，不是越高越好的连续刻度。**第六档「共同完成一件事」是顶端，而拆开会让它往下走——`add` 和币种校验分开之后，两个模块各自清楚，配合却脆弱了。
- **拆分不是无条件的目标。**只有拆完之后「总是一起变的东西」还待在一起，这次拆分才算数。
- **边界沿最可能变化的决策划，不要沿数据流划。**Parnas 1972 年就写过这条反面建议：按处理步骤切是多数人的第一反应。
- **等级表是量表，不是验收标准。**163 个人能给同一段代码标出不同等级，因为它依赖这个系统接下来往哪长。盯着档位调代码，可能越调越低。
